...
- 使用`resource_id`和`pid = 0`查询`comment_index`表，获取根评论，按`floor_count`排序。
- 对于每个评论，使用`id`从`comment_content`表获取相应的内容。
- 实现分页，以限制单个请求返回的评论数量，避免数据库过载。
- 使用分布式缓存（如Redis）缓存结果，以减少数据库负载，处理高流量。

#### 性能考虑

- 在`resource_id`，`pid`和`floor_count`上使用数据库索引，以加速查询。
- 实现数据库的读副本，以分发读取负载。
- 使用负载均衡器，将传入的请求分发到应用程序的多个实例。
